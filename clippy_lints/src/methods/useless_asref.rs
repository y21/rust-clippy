    ...
    if is_trait_method(cx, expr, sym::AsRef) || is_trait_method(cx, expr, sym::AsMut) {
        ...
        if base_rcv_ty == base_res_ty && rcv_depth >= res_depth {
            if let Some(parent) = get_parent_expr(cx, expr) {
                ...
                {
                    return;
                } else if let hir::ExprKind::AddrOf(hir::BorrowKind::Ref, ..) = parent.kind {
                    // allow the `as_ref` or `as_mut` if it is immediately borrowed,
                    // which causes Temporary Lifetime Extension to kick in.
                    // suggesting to remove `as_ref` would change behavior
                    return;
                }
            }
